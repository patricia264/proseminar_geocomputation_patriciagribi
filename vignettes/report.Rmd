...
atmosphere and solar activity, global temperatures should stabilize or even 
cooling should commence. Instead the temperature curve is rising and it is 
getting warmer (IPCC 2007, p.5).
...
The objective of this project was therefore to create a seamless and consistent
timeline that effectively portrays historical temperature and CO2
variations. The primary goal was to answer what steps need to be taken to
construct a time series and how the rate of change looks like.

By merging different data sets, a holistic view is achieved that
...
The data sets used in this research project are primarily sourced
from the National Oceanic and Atmospheric Administration (NOAA). They
are all tabular and come in either *.csv* or *.txt* format. The
age-scales vary as well as the resolution. The primary objective was to
...
resolution within the data sets. Additionally,
overlapping sections were identified and analyzed,
whether there are significant differences. In cases where a substantial
...
libraries. Subsequently, the data-frames were reduced to retain only the 
essential variables: year, CO2 concentrations and temperature records. 
To ensure uniformity and comparability, the data sets were then 
brought into identical format. This involved bringing all of them to the same 
age-scale, resolution, and units. With the data sets standardized, the analysis 
and the assembling of the timeline later on could be accomplished.
...
This standardization of variables enables the construction of a clear and
consistent timeline.
...
this constant value has been incrementally applied to each data set record.
...
800,000-year span. Like the co2-ice-cores data set the age is represented
...
set is expressed with decimal values as well.
...
degrees Celsius. Now the problem becomes evident, that the 800KYr data
...
years was conducted by using the other two temperature data sets. Following this
the absolute reference period temperature was added to the current data.
...
shows the overlapping period, where both, co2-ice-cores and
co2-mauna-loa, data are present.
...
ggplot() +
  ...
  labs(title = "Overlapping Section of the CO2 Data Sets",
       x = "Year",
       y = "CO2 [ppm]") +
...
lower concentrations of carbon dioxide in comparison to the
...
The figure above provides a very good initial overview of the
situation.
...
To determine the further procedure for creating the
time series, it was examined whether the two data sets show a
significant difference regarding their CO2 values during the overlapping
period.

For this purpose the mean over all data points of the overlapping period
have been conducted for the two data sets.

Before, the co2-ice-cores data set had to be brought to an
...
interpolated_df <- data.frame(
  # create sequence with annual resolution
...
The plot below shows the interpolated co2-ice-cores data set
together with the unchanged co2-mauna-loa data set. The ice-cores curve is much 
smoother and therefore shows fewer strong fluctuations.
...
ggplot() +
  ...
  geom_line(data = co2_mauna_loa, aes(x = year, y = co2, color = "Mauna Loa"), size = 1) +
  labs(title = "Overlapping Section with Corrected Ice-Cores Data Set",
       x = "Year",
       y = "CO2 [ppm]") +
  scale_color_manual(values = c("Ice Cores" = "cornflowerblue", "Mauna Loa" = "coral"),
                     name = "Datasources") +
...
determined. They were then subtracted from each other and this resulted in the
so-called offset. 
...
The t-test was carried out to see if there is a significant difference
between the means of the two data sets. The null-hypothesis states in this
case, that the two data-sets have no significant difference. The p-value
is ~ 0.68 and lies within the acceptance region. The null-hypothesis must
therefore be accepted and cannot be rejected. Consequently the two data
sets show no significant difference during the overlapping period. Based on
this result, the offset was not added to the ice-cores data set and
no correction was performed.


**Discussion:**

To construct the final timeline no offset was applied to the
ice-cores data as already specified. The interpolated values on the
other hand were adopted. Starting from the overlapping period, the
co2-mauna-loa data set was chosen. This decision was motivated by the
...
(maximum) ppm. However, in the latter part of the time series, a pronounced
increase is evident, surpassing all other fluctuations by a considerable
margin, reaching levels exceeding 400 ppm.
...
series was examined. As inferred from the plot above, the most
...
A step further could include to model future CO2. To do this more environmental
data and variables would be needed to capture this complex process of the 
increasing amount of co2 in the atmosphere. The figure above shows possible
model fits. The higher the polynomial, the better the modeled curve matches the 
actual data. But then there is the problem of overfitting. To find a suitable 
model the bias-variance-trade-off has to be considered.
...
three data sets. This emphasis was directed to facilitate the combination
...
ggplot() +
  ...
  geom_line(data = temp_NOAA_recentEra, aes(x = year, y = temp, color = "NOAA_recentEra"), size = 1) +
  geom_line(data = temp_800KYr, aes(x = year, y = temp, color = "800_KYr"), size = 1) +
  labs(title = "Temperature Overlapping Period",
       x = "Year",
       y = "Temperature [°C]") +
  scale_color_manual(values = c("pages2K" = "yellowgreen", "NOAA_recentEra" = "coral", "800_KYr" = "cornflowerblue"), name = "Datasources") +
  xlim(-1, 1912) +
  ylim(11,18) +
  theme_minimal()
...
focus is directed toward the green and orange data sets, due to their
closer time range and the similarity of both curves.

```{r, warning = FALSE, echo=FALSE}

# plot of the overlapping section of the 2 temperature data sets
...
ggplot() +
  ...
  labs(title = "Temperature Overlapping Period",
       x = "Year",
       y = "Temperature [°C]") +
  scale_color_manual(values = c("pages2K" = "yellowgreen", "NOAA_recentEra" = "coral"), name = "Datasources") +
  xlim(1849, 2001) +
  ylim(13,15) +
  theme_minimal()
...
region. The null-hypothesis was therefore accepted and could not be
rejected. The NOAA_recentEra and the pages2k data sets did not reveal a
...
meteorological stations globally, in contrast to the pages2k data
set, which relies on diverse global proxies. Consequently, the initial
segment of the temperature timeline was derived from the
...
ggplot(data = temp_timeline) +
  ...
  labs(title = "1. Part of the Temperature-Timeline",
       x = "Year",
       y = "Temperature [°C]") +
  scale_x_continuous(labels = label_comma()) + 
  theme_minimal()
...
The next step was to connect the 3rd, temp-800KYr, data set. Like
previously with the CO2 data, the first step was to check whether the
data set has large gaps. As this was the case (see code below), a linear
...
ggplot() +
  ...
  labs(title = "Temperature Overlapping Section",
       x = "Year",
       y = "Temperature [°C]") +
...
The p-value of the t-test here is very small and lies within the
...
ggplot() +
  geom_line(data= interpolated_800KYr, aes(x = year, y = temp, color = "800KYr"), size = 1) +
  geom_line(data = temp_timeline, aes(x = year, y = temp, color = "Temp-Timeline"), size = 1) +
  labs(title = "Temperature Overlapping Section with Corrected 800KYr Data Set",
       x = "Year",
       y = "Temperature [°C]") +
  scale_color_manual(values = c("800KYr" = "cornflowerblue", "Temp-Timeline" = "coral"),
                     name = "Datasources") +
...
ggplot(temp_timeline, aes(x = year, y = rate_of_change)) +
  geom_line() +
  geom_point() +
  labs(title = "Rate of Change of the Temperature Values",
       x = "Year",
       y = "Rate of Change") +
  scale_x_continuous(labels = label_comma()) +
  xlim(1500, 2023) +
...
- CLIMAP Project Members (1976): The Surface of the Ice-Age Earth. In: Science, New Series, Vol. 191, Issue 4232, pp. 1131-1137, 19 March 1974.
